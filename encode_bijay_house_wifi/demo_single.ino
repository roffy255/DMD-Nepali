...
const String name_comp PROGMEM = "JEWELLER SHOP SAMPLE DISPLAY";
...
void DecodeAndSaveBrightness(String In_val)
{
  ...
  EEPROM.write(250, screen_brightness);
}

void set_brightness()
{
  int brightness = EEPROM.read(250);
...
void show_date_time()
{
  ...
  dmd.drawString(1,8,get_time(":"));
  dmd.drawString(2,0,get_date(","));
  delay_engine(500);
  dmd.drawString(1,8,get_time(" "));
  dmd.drawString(2,0,get_date(","));
  delay_engine(500);
...
dmd.clearScreen();
  ...
  dmd.drawString(7, 0, "_");
  dmd.drawString(8, 8, silver);
...
{ dmd.selectFont(Arial_Black_16);
  String temp_buffer = name_comp;
    uint16_t string_length = temp_buffer.length();
  //  animation_whiteline_horizontal(0, 0, 32, true, true);
    ...
    {
        string_length = temp_buffer.length();
        temp_buffer = temp_buffer.substring(1, string_length);
        for(int j = 0; j > -10; j--)
        {
            dmd.drawString(j, 4, temp_buffer);
            delay(50);
        }
    }
