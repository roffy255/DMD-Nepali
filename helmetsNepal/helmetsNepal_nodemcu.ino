...
const char *ssidAP = "HelmetsNepal";
const char *passwordAP = "HelmetsNepal123";
...
void setup()
{
    ...
    server.on("/helmetsnepal_np", handleQueryArgs);
...
String get_time(String seperator)
{
    ...
    {
//        time_state = true;
         Time = valuee_hour + seperator + valuee_minute + seperator + valuee_second + " P.M.";
    }
    else
    {
//        time_state = false;
        Time = valuee_hour + seperator + valuee_minute + seperator + valuee_second + " A.M.";
    }
...
{
    dmd.selectFont(FONTEng);
     mid_display_engfont(0, 0, get_time(":"), 96);
     mid_display_engfont(8, 0, get_date(","), 96);
...
void loop()
{
    ...
    dmd.drawString(10, 1,"HELMETS");
    dmd.drawString(23, 16,"NEPAL");
    delay_engine(5000);
    motion.cleaningCurtain();
    dmd.selectFont(FONTEng);
    for(int i = 32; i > 0; i--){
    dmd.clearScreen();
    motion.helmet_emptyLeft(-2, i);
    motion.helmet_emptyRight(66, i);
    delay_engine(50);
    }

    for(int i = -2; i < 68; i++){
      dmd.clearScreen();
      motion.helmet_emptyLeft(i, 0);
      motion.helmet_emptyRight(64-i, 0);
      delay_engine(50); 
    }

    for(int i = 0; i < 10; i++){
     dmd.clearScreen();
     motion.helmet_emptyLeft(68, 0);
     motion.helmet_emptyRight(-3, 0);
    dmd.drawString(28, i, "HELMETS");
    dmd.drawString(34, 28-i, "NEPAL");
    delay_engine(50);
    }
    delay_engine(10000);
    motion.cleaningCurtain();
