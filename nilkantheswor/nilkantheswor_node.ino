...
const char *ssidAP = "Nilkantheswor";
const char *passwordAP = "NILKANTHESWOR123123";
const char *ota_hostname = "NILoilstore";
...
const String month_array[12] = {"BAISAKH", "JESTHA", "ASADH", "SHRAWAN", "BHADRA", "ASHOJ", "KARTIK", "MANGSIR", "POUSH", "MAGH", "FALGUN", "CHAITRA"};
...
void setup()
{
    ...
    if (!OTA_flag)
    {
        ...
        server.on("/fcc25e0fb6c1e8e58ada82a63e718bcb", handleQueryArgs);
...
{ //Handler
    for (int i = 0; i < server.args(); i++)
    {
        ...
        {
            EEPROM.write(505, (value.substring(0, value.indexOf("."))).toInt());
            EEPROM.commit();
            EEPROM.write(506, (value.substring(value.indexOf("."), value.length())).toInt());
            EEPROM.commit();
        }
        else if (key == "D")
        {
            EEPROM.write(507, (value.substring(0, value.indexOf("."))).toInt());
            EEPROM.commit();
            EEPROM.write(508, (value.substring(value.indexOf("."), value.length())).toInt());
            EEPROM.commit();
        }
    }
...
{
    dmd.selectFont(FONTEng);
    mid_display_engfont(16, 0, get_date(), 96);
    dmd.drawString(7 - 3, 25 - 1, get_time(":"));
    dmd.setPixel(87 - 3, 25 - 1, GRAPHICS_ON);
    dmd.setPixel(88 - 3, 25 - 1, GRAPHICS_ON);
    dmd.setPixel(87 - 3, 26 - 1, GRAPHICS_ON);
    dmd.setPixel(88 - 3, 26 - 1, GRAPHICS_ON);
    dmd.drawString(87, 25 - 1, "C");
    delay_engine(500);
    mid_display_engfont(16, 0, get_date(), 96);
    dmd.drawString(7 - 3, 25 - 1, get_time(" "));
    delay_engine(500);
...
{
    motion.petrol(32, 0);
    dmd.drawString(32, 8, String(EEPROM.read(505)) + "." + String(EEPROM.read(506)).substring(0, 1));
}

void get_diesel()
{
    motion.diesel(64, 0);
    dmd.drawString(64, 8, String(EEPROM.read(507)) + "." + String(EEPROM.read(508)).substring(0, 1));
}
